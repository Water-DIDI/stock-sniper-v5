...
# ==========================================
# Tab 1: 板塊戰情室 (完整熱力圖 + 趨勢)
# ==========================================
with tab1:
    st.markdown("### 資金流向熱力榜 (Sector Heatmap)")
    try:
        with st.spinner('正在分析華爾街資金流向...'):
            df_close = get_sector_data()

        # 定義完整週期 (1, 3, 6, 9, 12 個月)
        periods = {
            '1M (近1月)': 21,
            '3M (近1季)': 63,
            '6M (半年)': 126,
        ...
        st.caption("💡 排序依據：近1個月 (1M) 強弱。顏色越綠代表資金流入越強。")

        # 互動趨勢圖
        st.markdown("---")
        st.subheader("📈 板塊趨勢對比 (Trend Chart)")
        
        default_sectors = ['XLE', 'XLK', 'SMH', 'XLU']
        selected = st.multiselect("選擇板塊:", list(SECTOR_MAP.keys()), default=[k for k in default_sectors if k in SECTOR_MAP])
        ...
        if selected:
            ...
            fig = px.line(chart_data, title=f"近 {lookback} 天累積報酬率 (%)")
    ...
    if 'df_close' in locals() and not df_close.empty:
        ...
        for i, ticker in enumerate(key_sectors):
            if ticker in df_close:
                ...
                # 只顯示 ticker 名稱比較簡潔
                with cols[i]:
                    st.metric(label=f"{ticker} ({SECTOR_MAP[ticker].split(' ')[0]})", value=f"{price:.2f}", delta=emoji)
        st.markdown("---")
